...
ENTERPRISE_CONFIG = {
    ...
    'CALL_VOLUME_FILE': r'data\call_volumes.csv',           # Primary for correlations (Date column)
    'CALL_INTENTS_FILE': r'data\GenesysExtract_with_intents.csv',  # Secondary for intent analysis
    'MAIL_FILE_PATH': r'merged_output.csv',                 # Mail data
    
    # COLUMN MAPPINGS
    'MAIL_COLUMNS': {'date': 'mail_date', 'volume': 'mail_volume', 'type': 'mail_type'},
    'CALL_VOLUME_COLUMNS': {'date': 'Date', 'volume': 'call_volume'},  # Call volumes file
...
class EnterpriseLogger:
    ...
    def __init__(self, name: str = "CustomerCommsDashboard", level: LogLevel = LogLevel.INFO):
        self.name = name
        self.level = level
    ...
    def _setup_logger(self) -> logging.Logger:
        """Setup enterprise logging with multiple handlers and formatters"""
    ...
    def critical(self, message: str, exception: Exception = None):
        ...
        if exception:
            error_msg += f" | Exception: {str(exception)} | Traceback: {traceback.format_exc()}"
...
class PremiumDashboardVisualizations:
    ...
    def create_premium_overview_chart(self, df: pd.DataFrame) -> go.Figure:
        ...
        try:
            ...
            financial_colors = [self.theme.primary_purple, self.theme.primary_teal, self.theme.accent_gold]
