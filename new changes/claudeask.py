...
# Assuming THEME and ENTERPRISE_CONFIG are defined globally or passed in
# from .config import THEME, ENTERPRISE_CONFIG
# from .data_processor import EnterpriseDataProcessor # Assuming this import path

# Re-defining placeholder for THEME and ENTERPRISE_CONFIG for self-contained code block
# In your actual file, these would be imported or defined earlier.
from dataclasses import dataclass
from enum import Enum
...
ENTERPRISE_CONFIG = {
    'LOG_FILE_PATH': r'logs\dashboard.log',
    'LOG_NAME': 'CustomerCommsDashboard',
    'CALL_VOLUME_FILE': r'data\GenesysExtract_20250703.csv',
    'CALL_INTENTS_FILE': r'data\GenesysExtract_20250609.csv',
    'MAIL_FILE_PATH': r'merged_output.csv',
    'MAIL_COLUMNS': {'date': 'mail_date', 'volume': 'mail_volume', 'type': 'mail_type'},
    'CALL_VOLUME_COLUMNS': {'date': 'Date'},
    'CALL_INTENT_COLUMNS': {'date': 'ConversationStart', 'intent': 'uui_Intent'},
...
# Placeholder for EnterpriseDataProcessor to allow this block to be self-contained
class EnterpriseDataProcessor:
    def __init__(self, config: Dict, logger):
        ...
        self.call_intent_df = pd.DataFrame()
        self.financial_df = pd.DataFrame()
        self.has_mail_data = False
        self.mail_by_type = pd.DataFrame()
        self.anomaly_data = pd.DataFrame()
        self.decomposition_results = {}
        # Dummy data for demonstration if not loaded
        self.combined_df = pd.DataFrame({
            'date': pd.to_datetime(pd.date_range(start='2024-01-01', periods=100)),
            'call_volume': np.random.randint(1000, 5000, 100),
            'mail_volume': np.random.randint(5000, 15000, 100),
            'call_volume_normalized': np.random.randn(100),
            'mail_volume_normalized': np.random.randn(100),
            'S&P 500_normalized': np.random.randn(100),
            '10-Year Treasury_normalized': np.random.randn(100),
            'VIX_normalized': np.random.randn(100),
            'calls_per_1k_mails': np.random.rand(100) * 100,
            'weekday': [d.day_name() for d in pd.date_range(start='2024-01-01', periods=100)],
        })
        self.has_mail_data = True # Assume mail data is available for viz
        self.mail_by_type = pd.DataFrame({'type': ['TypeA', 'TypeB'], 'volume': [100, 200]})
        self.anomaly_data = pd.DataFrame({
            'date': [pd.to_datetime('2024-03-15')],
            'call_volume': [6000],
            'mail_volume': [2000],
            'anomaly_score': [-0.5],
            'anomaly_reason': ['Unusual spike']
        })
        self.decomposition_results = {
            'call_volume': {
                'trend': pd.Series(np.random.rand(100), index=self.combined_df['date']),
                'seasonal': pd.Series(np.random.rand(100), index=self.combined_df['date']),
                'residual': pd.Series(np.random.rand(100), index=self.combined_df['date']),
                'observed': pd.Series(np.random.rand(100), index=self.combined_df['date'])
            }
        }
        self.logger.info("Data Processor initialized with dummy data for visualization.")
...
class PremiumDashboardVisualizations:
    ...
    def __init__(self, data_processor: EnterpriseDataProcessor):
        ...
        # Common layout settings - REMOVED 'title' from here
        self.base_layout = {
            ...
            # Removed 'title' from base_layout to avoid duplicate argument error
            'legend': {
                'orientation': 'h',
                'yanchor': 'bottom',
                'y': 1.02,
    ...
    def create_premium_overview_chart(self, df: pd.DataFrame) -> go.Figure:
        ...
        try:
            ...
            fig = make_subplots(
                rows=2, cols=1,
                subplot_titles=('📊 Executive Overview: Communications Intelligence & Market Context',
                                '📈 Daily Volume Analysis with Trend Indicators'),
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    mode='lines+markers',
                    marker={'size': 8, 'color': self.theme.primary_rose},
            ...
            if len(df) >= 20:
                ...
                fig.add_trace(
                    go.Scatter(
                        ...
                        name='Call Trend',
                        line={
                            'color': self.theme.accent_gold, # Changed from accent_rose to accent_gold
                            'width': 3,
                            'dash': 'dash'
                        },
                        showlegend=False
            ...
            # Premium styling
            fig.update_layout(
                title={ # This is now the ONLY 'title' being passed explicitly
                    'text': '<b>🎯 Customer Communications Intelligence Dashboard</b><br><sub>Advanced Analytics with Market Context & Trend Analysis</sub>',
                    'font': {
                        'size': 28,
                ...
                **self.base_layout # base_layout no longer contains 'title'
            )
            ...
            fig.update_yaxes(
                ...
                title_font={'size': 16, 'color': self.theme.primary_rose},
                row=2, col=1, secondary_y=True
            )
            ...
            self.logger.error("❌ Premium overview chart creation failed", e)
    ...
    def create_lag_analysis_heatmap(self, df: pd.DataFrame) -> go.Figure:
        ...
        try:
            ...
            fig.update_layout(
                title={
                    'text': '<b>📊 Mail Type Lag Analysis Heatmap</b><br><sub>Correlation between mail types and call volume across time lags</sub>',
            ...
            self.logger.error("❌ Lag analysis heatmap creation failed", e)
    ...
    def create_scatter_with_trend(self, df: pd.DataFrame) -> go.Figure:
        ...
        try:
            ...
            # Linear regression
            if hasattr(self, 'SKLEARN_AVAILABLE') and self.SKLEARN_AVAILABLE: # Use hasattr for safety
                from sklearn.linear_model import LinearRegression
                from sklearn.metrics import r2_score
                lr = LinearRegression()
            ...
            if hasattr(self, 'SKLEARN_AVAILABLE') and self.SKLEARN_AVAILABLE and len(scatter_data) >= 20:
            ...
            fig.update_layout(
                title={
                    'text': '<b>📈 Predictive Scatter Analysis</b><br><sub>Mail Volume vs Next Day Call Volume with Trend Analysis</sub>',
                    'font': {'size': 24, 'color': self.theme.neutral_900},
                    'x': 0.5
                },
            ...
            self.logger.error("❌ Scatter with trend creation failed", e)
    ...
    def create_time_series_decomposition(self, df: pd.DataFrame) -> go.Figure:
        ...
        try:
            ...
            fig.update_layout(
                title={
                    'text': '<b>📊 Time Series Decomposition Analysis</b><br><sub>Trend, Seasonal, and Residual Components of Call Volume</sub>',
                    'font': {'size': 24, 'color': self.theme.neutral_900},
                    'x': 0.5
                },
                height=self.config['CHART_HEIGHT_LARGE'],
            ...
            self.logger.error("❌ Time series decomposition chart creation failed", e)
    ...
    def create_anomaly_detection_plot(self, df: pd.DataFrame) -> go.Figure:
        ...
        try:
            ...
            if not anomaly_data.empty:
                ...
                for _, anomaly in top_anomalies.iterrows():
                    fig.add_annotation(
                        ...
                        text=f"⚠️ {anomaly['anomaly_reason'][:30]}...",
                        showarrow=True,
                        arrowhead=2,
                        arrowsize=1,
                        arrowwidth=2,
            ...
            fig.update_layout(
                title={
                    'text': '<b>🚨 Anomaly Detection Analysis</b><br><sub>Identifying unusual patterns in mail-call relationships</sub>',
                    'font': {'size': 24, 'color': self.theme.neutral_900},
                    'x': 0.5
                },
            ...
            self.logger.error("❌ Anomaly detection plot creation failed", e)
...
class EnterpriseLogger:
    ...
    def __init__(self, config: Dict):
        ...
        self.logger.info(f"🚀 Logger initialized. Log file: {os.path.abspath(self.log_file_path)}")
        ...
        try:
            ...
            for encoding in encodings:
                try:
                    ...
                    if not df.empty:
                        self.logger.info(f"✅ {description} loaded successfully", {
                            'file_path': file_path,
                            'records': len(df),
            ...
            self.logger.error(f"❌ Failed to load {description} with any encoding")
            return pd.DataFrame()
            
        except Exception as e:
            self.logger.error(f"❌ Critical error loading {description}", e)
            return pd.DataFrame()
    ...
    def safe_date_conversion(self, df: pd.DataFrame, date_column: str, 
        ...
        try:
            ...
            if not successful_conversion:
                self.logger.error(f"❌ Could not convert {date_column} to datetime in {data_type}")
                return df
...
def main():
    ...
    """
    print("🔥 CUSTOMER COMMUNICATIONS INTELLIGENCE DASHBOARD v3.0")
    print("=" * 80)
    print("🎯 ENTERPRISE FEATURES:")
    print("   • Dual call data source management (volume + intents)")
    print("   • Mail data filtered to call data presence")
    print("   • 4 NEW advanced plots: Lag Analysis, Scatter Trends, Time Series Decomposition, Anomaly Detection")
    print("   • Premium UX/UI with enhanced styling and interactivity")
    print("   • Production-grade error handling and logging")
    print("   • Enhanced correlation analysis with multiple feature types")
    print("   • Financial data normalization and proper scaling")
    print("   • Top 10 + Other grouping for cleaner visualizations")
    print("=" * 80)
    ...
    try:
        ...
        if not DASH_AVAILABLE or not PLOTLY_AVAILABLE:
            logger.critical("❌ Required dependencies missing")
            print("❌ DEPENDENCY ERROR: Missing required packages")
            print("📦 Install with: pip install plotly dash dash-bootstrap-components")
            if not SCIPY_AVAILABLE:
                print("📦 For advanced analytics: pip install scipy")
            if not SKLEARN_AVAILABLE:
                print("📦 For ML features: pip install scikit-learn")
            if not STATSMODELS_AVAILABLE:
                print("📦 For time series: pip install statsmodels")
            return False
        ...
        if not pipeline_success:
            logger.error("❌ Master data pipeline failed")
            return False
        
        # Initialize dashboard application
        logger.info("🎨 Initializing Premium Dashboard Application...")
        dashboard = EnterpriseDashboardApp(dp)
        
        # Generate comprehensive status report
        logger.info("=" * 80)
        logger.info("📊 ENTERPRISE DASHBOARD v3.0 STATUS REPORT")
        logger.info("=" * 80)
        
        # Data status
        logger.info("📥 DATA SOURCES:")
        logger.info(f"   • Call Volume Data: {'✅ LOADED' if dp.has_call_volume_data else '❌ MISSING'} ({len(dp.call_volume_df):,} records)")
        logger.info(f"   • Call Intent Data: {'✅ LOADED' if dp.has_call_intent_data else '❌ MISSING'} ({len(dp.call_intent_df):,} records)")
        logger.info(f"   • Mail Data: {'✅ LOADED' if dp.has_mail_data else '❌ MISSING'} ({len(dp.mail_df):,} records)")
        logger.info(f"   • Financial Data: {'✅ LOADED' if dp.has_financial_data else '❌ MISSING'} ({len(dp.financial_df):,} records)")
        
        # Processing status
        logger.info("⚙️ DATA PROCESSING:")
        logger.info(f"   • Combined Dataset: {len(dp.combined_df):,} business day records")
        logger.info(f"   • Date Range: {dp.combined_df['date'].min().date()} to {dp.combined_df['date'].max().date()}")
        logger.info(f"   • Weekends Removed: {dp.data_quality_metrics.get('weekends_removed', 0):,}")
        logger.info(f"   • Holidays Removed: {dp.data_quality_metrics.get('holidays_removed', 0):,}")
        logger.info(f"   • Outliers Removed: {dp.data_quality_metrics.get('outliers_removed', 0):,}")
        
        # Analytics status
        logger.info("🔬 ANALYTICS RESULTS:")
        logger.info(f"   • Momentum Correlations: {len(dp.momentum_correlation_results):,} calculated")
        logger.info(f"   • Rolling Correlations: {len(dp.rolling_correlation_results):,} windows analyzed")
        logger.info(f"   • Intent Correlations: {len(dp.intent_correlation_results):,} intents analyzed")
        logger.info(f"   • Anomalies Detected: {len(dp.anomaly_data):,} anomalous patterns")
        logger.info(f"   • Time Series Decomposition: {'✅ COMPLETED' if dp.decomposition_results else '❌ FAILED'}")
        
        # Key insights
        if not dp.momentum_correlation_results.empty:
            best_corr = dp.momentum_correlation_results.iloc[0]
            logger.info("🎯 KEY INSIGHTS:")
            logger.info(f"   • Best Correlation: {best_corr['correlation']:.3f} ({best_corr['feature']})")
            logger.info(f"   • Optimal Lag: {best_corr['lag_days']} days")
            logger.info(f"   • Feature Category: {best_corr['category']}")
        ...
        if dp.efficiency_metrics:
            ...
            logger.info(f"   • Average Daily Calls: {dp.efficiency_metrics['avg_daily_calls']:.0f}")
            logger.info(f"   • Average Daily Mail: {dp.efficiency_metrics['avg_daily_mail']:.0f}")
            logger.info(f"   • Data Quality Score: {dp.efficiency_metrics['data_quality_score']:.0f}%")
        
        # Feature status
        logger.info("🎯 FEATURE STATUS:")
        logger.info(f"   • Lag Analysis Heatmap: {'✅ AVAILABLE' if dp.has_mail_data else '❌ REQUIRES MAIL DATA'}")
        logger.info(f"   • Scatter Trend Analysis: {'✅ AVAILABLE' if len(dp.combined_df) > 10 else '❌ INSUFFICIENT DATA'}")
        logger.info(f"   • Time Series Decomposition: {'✅ AVAILABLE' if dp.decomposition_results else '❌ REQUIRES MORE DATA'}")
        logger.info(f"   • Anomaly Detection: {'✅ AVAILABLE' if not dp.anomaly_data.empty else '❌ NO ANOMALIES DETECTED'}")
        logger.info(f"   • Intent Analysis: {'✅ AVAILABLE' if dp.has_call_intent_data else '❌ REQUIRES INTENT DATA'}")
        
        logger.info("=" * 80)
        logger.info("🌐 DASHBOARD ACCESS:")
        logger.info("   • URL: http://127.0.0.1:8050")
        logger.info("   • Status: Ready for executive presentation")
...
if __name__ == '__main__':
    ...
    if critical_missing:
        ...
        print("\n🚫 Cannot start dashboard without critical dependencies.")
        sys.exit(1)
    
    print("✅ All critical dependencies available!")
    print("🚀 Starting Enterprise Dashboard v3.0...\n")
    
    # Execute main function
    success = main()
    
    if success:
        print("\n✅ Enterprise Dashboard v3.0 session completed successfully")
        print("🎯 Thank you for using Customer Communications Intelligence Platform")
    else:
...
================================================================================
15:26:48 | INFO | 🚀 Logger initialized. Log file: C:\Users\BhungarD\OneDrive - Computershare\Desktop\prod - mail call analysis and intent\logs\dashboard.log
15:26:48 | INFO | 🏗️ Initializing Enterprise Data Processor v3.0...
15:26:48 | INFO | Enterprise Data Processor v3.0 initialized
15:26:48 | INFO | 📥 Phase 1: Data Loading
15:26:48 | INFO | 🔄 Loading primary call volume data...
15:26:53 | INFO | ✅ Call Volume Data loaded successfully | Data: {"file_path": "data\\GenesysExtract_20250703.csv", "records": 1043025, "columns": 25, "encoding": "utf-8", "file_size_mb": 132.89}
15:26:53 | INFO | Date conversion completed for call volume | Data: {"success_rate": "100.0%", "valid_records": 1043025, "original_records": 1043025, "date_range": "2024-01-01 to 2025-07-03"}
15:26:54 | INFO | ✅ Call volume data loaded successfully | Data: {"total_records": 284, "date_range": "2024-06-03 to 2025-07-03", "avg_daily_volume": "3199.7", "filtered_out": 266}
15:26:54 | INFO | 🔄 Loading secondary call intent data...
15:27:06 | INFO | ✅ Call Intent Data loaded successfully | Data: {"file_path": "data\\GenesysExtract_20250609.csv", "records": 1053601, "columns": 42, "encoding": "utf-8", "file_size_mb": 307.2}
15:27:08 | ERROR | ❌ Could not convert ConversationStart to datetime in call intent
15:27:10 | WARNING | Insufficient call intent data: 0 records
15:27:10 | INFO | 🔄 Loading mail data...
15:27:12 | INFO | ✅ Mail Data loaded successfully | Data: {"file_path": "merged_output.csv", "records": 1414794, "columns": 4, "encoding": "utf-8", "file_size_mb": 107.72}
15:27:12 | INFO | Date conversion completed for mail | Data: {"success_rate": "0.4%", "valid_records": 5014, "original_records": 1414794, "date_range": "2024-07-03 to 2025-05-30"}
15:27:12 | INFO | Filtered mail data to call dates: 5014 records (was 5014)
15:27:12 | INFO | ✅ Mail data loaded successfully | Data: {"total_records": 3448, "unique_types": 20, "date_range": "2024-07-03 to 2025-05-30", "total_volume": "17,035,835", "avg_daily_volume": "4940.8", "filtered_out": 1566}
15:27:12 | INFO | 🔄 Loading financial indicator data...
15:27:12 | INFO | Loading financial data from 2024-05-29 to 2025-07-04
15:27:12 | INFO | Fetching S&P 500 (^GSPC)...
15:27:13 | INFO | ✅ S&P 500: 401 data points loaded
15:27:13 | INFO | Fetching 10-Year Treasury (^TNX)...
15:27:13 | INFO | ✅ 10-Year Treasury: 401 data points loaded
15:27:13 | INFO | Fetching VIX (^VIX)...
15:27:13 | INFO | ✅ VIX: 401 data points loaded
15:27:13 | INFO | Fetching Dollar Index (DX-Y.NYB)...
15:27:13 | INFO | ✅ Dollar Index: 401 data points loaded
15:27:13 | INFO | Fetching Gold (GC=F)...
15:27:13 | INFO | ✅ Gold: 401 data points loaded
15:27:13 | INFO | Fetching Oil (CL=F)...
15:27:13 | INFO | ✅ Oil: 401 data points loaded
15:27:13 | INFO | ✅ Financial data loaded successfully | Data: {"indicators": 6, "total_records": 664, "date_range": "2024-05-29 to 2025-07-03", "indicators_loaded": ["S&P 500", "10-Year Treasury", "VIX", "Dollar Index", "Gold", "Oil"]}
15:27:13 | INFO | ⚙️ Phase 2: Data Combination and Processing
15:27:13 | INFO | 🔄 Combining and processing data...
15:27:13 | INFO | Inner join result: 146 records with both call and mail data
15:27:13 | INFO | Added 6 financial indicators
15:27:13 | INFO | Removed 2 outliers from call_volume
15:27:13 | INFO | Removed 34 outliers from mail_volume
15:27:13 | INFO | ✅ Data combination completed successfully | Data: {"final_records": 288, "date_range": "2024-07-03 to 2025-05-30", "features_created": 18, "data_quality": {"total_records_processed": 0, "records_after_filtering": 288, "weekends_removed": 114, "holidays_removed": "0", "outliers_removed": "36", "missing_data_imputed": 0}}
15:27:13 | INFO | 🔬 Phase 3: Advanced Analytics
15:27:13 | INFO | 🔄 Analyzing enhanced correlations...
15:27:13 | INFO | Significant correlation found: mail_momentum_3d lag 20d = 0.160 (p=0.009)
15:27:13 | INFO | Significant correlation found: mail_momentum_14d lag 4d = -0.151 (p=0.011)
15:27:13 | INFO | Significant correlation found: mail_rolling_mean_7d lag 0d = -0.211 (p=0.000)
15:27:13 | INFO | Significant correlation found: mail_rolling_mean_7d lag 2d = -0.226 (p=0.000)        
15:27:13 | INFO | Significant correlation found: mail_rolling_mean_7d lag 4d = -0.251 (p=0.000)        
15:27:13 | INFO | Significant correlation found: mail_rolling_mean_7d lag 6d = -0.246 (p=0.000)        
15:27:13 | INFO | Significant correlation found: mail_rolling_mean_7d lag 8d = -0.226 (p=0.000)        
15:27:13 | INFO | Significant correlation found: mail_rolling_mean_7d lag 10d = -0.214 (p=0.000)       
15:27:13 | INFO | Significant correlation found: mail_rolling_mean_7d lag 12d = -0.220 (p=0.000)       
15:27:13 | INFO | Significant correlation found: mail_rolling_mean_7d lag 14d = -0.230 (p=0.000)       
15:27:13 | INFO | Significant correlation found: mail_rolling_mean_7d lag 16d = -0.235 (p=0.000)       
15:27:13 | INFO | Significant correlation found: mail_rolling_mean_7d lag 18d = -0.219 (p=0.000)
15:27:13 | INFO | Significant correlation found: mail_rolling_mean_7d lag 20d = -0.190 (p=0.002)       
15:27:13 | INFO | Significant correlation found: mail_rolling_mean_14d lag 0d = -0.370 (p=0.000)       
15:27:13 | INFO | Significant correlation found: mail_rolling_mean_14d lag 2d = -0.374 (p=0.000)       
15:27:13 | INFO | Significant correlation found: mail_rolling_mean_14d lag 4d = -0.382 (p=0.000)       
15:27:13 | INFO | Significant correlation found: mail_rolling_mean_14d lag 6d = -0.373 (p=0.000)       
15:27:13 | INFO | Significant correlation found: mail_rolling_mean_14d lag 8d = -0.370 (p=0.000)
15:27:13 | INFO | Significant correlation found: mail_rolling_mean_14d lag 10d = -0.358 (p=0.000)      
15:27:13 | INFO | Significant correlation found: mail_rolling_mean_14d lag 12d = -0.349 (p=0.000)      
15:27:13 | INFO | Significant correlation found: mail_rolling_mean_14d lag 14d = -0.341 (p=0.000)      
15:27:13 | INFO | Significant correlation found: mail_rolling_mean_14d lag 16d = -0.329 (p=0.000)      
15:27:13 | INFO | Significant correlation found: mail_rolling_mean_14d lag 18d = -0.317 (p=0.000)      
15:27:13 | INFO | Significant correlation found: mail_rolling_mean_14d lag 20d = -0.289 (p=0.000)
15:27:13 | INFO | ✅ Enhanced correlation analysis completed | Data: {"total_correlations": 121, "significant_correlations": 46, "best_correlation": -0.3817708619972869, "best_feature": "mail_rolling_mean_14d"}
15:27:13 | INFO | 🔄 Analyzing rolling correlation patterns...
15:27:16 | INFO | ✅ Rolling correlation analysis completed | Data: {"total_windows": 1517, "lags_tested": 6, "window_size": 30, "summary_stats": {"lag_0d": {"mean": -0.17652625962406088, "std": 0.31915203673206466, "max": 0.7715346229973337, "min": -0.8432322947924075}, "lag_1d": {"mean": -0.17621931523104872, "std": 0.3230637987012185, "max": 0.7723171697545114, "min": -0.8427508941473336}, "lag_3d": {"mean": -0.1764075726648726, "std": 0.33191610614443895, "max": 0.7740592201004308, "min": -0.8417405359459534}, "lag_5d": {"mean": -0.17558800964534468, "std": 0.34104257346013267, "max": 0.8101804483371241, "min": -0.8480536230568501}, "lag_7d": {"mean": -0.17585892142555634, "std": 0.34988038531262056, "max": 0.836707136495966, "min": -0.8560904270945998}, "lag_14d": {"mean": -0.18492423069240843, "std": 0.39298100329773383, "max": 0.8893961944713438, "min": -0.8550234461872208}}}
15:27:16 | WARNING | Intent correlation analysis skipped - missing data or dependencies
15:27:16 | INFO | ✅ Efficiency metrics calculated | Data: {"metrics_calculated": 13, "response_rate": "7.41%", "best_correlation": -0.3817708619972869, "data_quality_score": 95}
15:27:16 | INFO | 🎯 Phase 4: Advanced Features
15:27:16 | INFO | 🔄 Detecting anomalies in communication patterns...
15:27:17 | INFO | ✅ Anomaly detection completed | Data: {"total_anomalies": 29, "anomaly_rate": "10.1%", "most_anomalous_date": "2024-12-16", "features_used": ["call_volume", "mail_volume", "calls_per_1k_mails", "mail_momentum_3d", "mail_momentum_7d"]}
15:27:17 | INFO | 🔄 Performing time series decomposition...
15:27:17 | INFO | ✅ Call volume decomposition completed
15:27:17 | INFO | ✅ Mail volume decomposition completed
15:27:17 | INFO | ✅ Time series decomposition completed | Data: {"series_decomposed": 2, "decomposition_stats": {"call_volume": {"trend_strength": 0.9580995784763755, "seasonal_strength": 0.01576101362067151, "residual_strength": 0.2286750349639586}, "mail_volume": {"trend_strength": 0.5509237241699158, "seasonal_strength": 0.09026643285884625, "residual_strength": 0.81316211855089}}}
15:27:17 | INFO | ✅ All processing phases completed successfully
15:27:17 | INFO | 🎨 Initializing Premium Dashboard Application...
15:27:17 | ERROR | ❌ Premium overview chart creation failed | Exception: plotly.graph_objs._figure.Figure.update_layout() got multiple values for keyword argument 'title'
Traceback (most recent call last):
  File "c:\Users\BhungarD\OneDrive - Computershare\Desktop\prod - mail call analysis and intent\eda.py", line 3064, in create_premium_overview_chart
    fig.update_layout(
    ~~~~~~~~~~~~~~~~~^
        title={
        ^^^^^^^
...
15:27:18 | INFO | ✅ Enterprise Dashboard Application v3.0 initialized
15:27:18 | INFO | ================================================================================     
15:27:18 | INFO | 📊 ENTERPRISE DASHBOARD v3.0 STATUS REPORT
15:27:18 | INFO | ================================================================================     
15:27:18 | INFO | 📥 DATA SOURCES:
15:27:18 | INFO |    • Call Volume Data: ✅ LOADED (284 records)
15:27:18 | INFO |    • Call Intent Data: ❌ MISSING (0 records)
15:27:18 | INFO |    • Mail Data: ✅ LOADED (3,448 records)
15:27:18 | INFO |    • Financial Data: ✅ LOADED (664 records)
15:27:18 | INFO | ⚙️ DATA PROCESSING:
15:27:18 | INFO |    • Combined Dataset: 288 business day records
15:27:18 | INFO |    • Date Range: 2024-07-03 to 2025-05-30
15:27:18 | INFO |    • Weekends Removed: 114
15:27:18 | INFO |    • Holidays Removed: 0
15:27:18 | INFO |    • Outliers Removed: 36
15:27:18 | INFO | 🔬 ANALYTICS RESULTS:
15:27:18 | INFO |    • Momentum Correlations: 121 calculated
15:27:18 | INFO |    • Rolling Correlations: 1,517 windows analyzed
15:27:18 | INFO |    • Intent Correlations: 0 intents analyzed
15:27:18 | INFO |    • Anomalies Detected: 29 anomalous patterns
15:27:18 | INFO |    • Time Series Decomposition: ✅ COMPLETED
15:27:18 | INFO | 🎯 KEY INSIGHTS:
15:27:18 | INFO |    • Best Correlation: -0.382 (mail_rolling_mean_14d)
15:27:18 | INFO |    • Optimal Lag: 4 days
15:27:18 | INFO |    • Feature Category: rolling_mean
15:27:18 | INFO |    • Top 3 Correlations:
15:27:18 | INFO |      1. mail_rolling_mean_14d (4d): -0.382
15:27:18 | INFO |      2. mail_rolling_mean_14d (2d): -0.374
15:27:18 | INFO |      3. mail_rolling_mean_14d (6d): -0.373
15:27:18 | INFO | 📈 PERFORMANCE METRICS:
15:27:18 | INFO |    • Response Rate: 7.41%
15:27:18 | INFO |    • Average Daily Calls: 3227
15:27:18 | INFO |    • Average Daily Mail: 43537
15:27:18 | INFO |    • Data Quality Score: 95%
15:27:18 | INFO | 🎯 FEATURE STATUS:
15:27:18 | INFO |    • Lag Analysis Heatmap: ✅ AVAILABLE
15:27:18 | INFO |    • Scatter Trend Analysis: ✅ AVAILABLE
15:27:18 | INFO |    • Time Series Decomposition: ✅ AVAILABLE
15:27:18 | INFO |    • Anomaly Detection: ✅ AVAILABLE
15:27:18 | INFO |    • Intent Analysis: ❌ REQUIRES INTENT DATA
15:27:18 | INFO | ================================================================================     
15:27:18 | INFO | 🌐 DASHBOARD ACCESS:
15:27:18 | INFO |    • URL: http://127.0.0.1:8050
15:27:18 | INFO |    • Status: Ready for executive presentation
15:27:18 | INFO | 🛑 Press Ctrl+C to stop the dashboard
15:27:18 | INFO | ================================================================================     
15:27:18 | INFO | 🚀 Starting Enterprise Dashboard v3.0 at http://127.0.0.1:8050
Dash is running on http://127.0.0.1:8050/

 * Serving Flask app 'eda'
 * Debug mode: off
WARNING: This is a development server. Do not use it in a production deployment. Use a production WSGI server instead.
 * Running on http://127.0.0.1:8050
Press CTRL+C to quit
127.0.0.1 - - [09/Jul/2025 15:27:19] "GET /_reload-hash HTTP/1.1" 200 -
127.0.0.1 - - [09/Jul/2025 15:27:22] "GET /_reload-hash HTTP/1.1" 200 -
127.0.0.1 - - [09/Jul/2025 15:27:25] "GET /_reload-hash HTTP/1.1" 200 -
127.0.0.1 - - [09/Jul/2025 15:27:27] "GET / HTTP/1.1" 200 -
127.0.0.1 - - [09/Jul/2025 15:27:27] "GET /_dash-layout HTTP/1.1" 200 -
127.0.0.1 - - [09/Jul/2025 15:27:27] "GET /_dash-dependencies HTTP/1.1" 200 -
127.0.0.1 - - [09/Jul/2025 15:27:27] "GET /_dash-component-suites/dash/dcc/async-graph.js HTTP/1.1" 304 -
127.0.0.1 - - [09/Jul/2025 15:27:27] "GET /_dash-component-suites/plotly/package_data/plotly.min.js HTTP/1.1" 304 -
127.0.0.1 - - [09/Jul/2025 15:27:28] "GET /_reload-hash HTTP/1.1" 200 -
127.0.0.1 - - [09/Jul/2025 15:27:31] "GET /_reload-hash HTTP/1.1" 200 -
