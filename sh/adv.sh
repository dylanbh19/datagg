#!/usr/bin/env bash
# =====================================================================
#  setup_full_pipeline.sh   v2 (2025-07-11)
#  • ONE SHOT: create / refresh customer_comms package
#  • UTF-8 logging on Windows
#  • Weekday-only analysis
#  • Generates 13 artefacts (5 stage-0…4 + 4 extras + 3 correlations
#    + data-gap visual)
# =====================================================================
set -euo pipefail
...
class Settings(BaseSettings):
    ...
    min_rows:   int  = 20
    max_lag:    int  = 21          # used in lag_scan & heat-map

settings = Settings()
PY

# ---------------------------------------------------------------------
# 3.  logging_utils.py  – UTF-8 safe
# ---------------------------------------------------------------------
...
def _read(p: Path)->pd.DataFrame:
    ...
    for enc in _ENCODINGS:
        try:
            ...
            continue
    log.error(f"Cannot decode {p}")
    return pd.DataFrame()

_to_date = lambda s: (pd.to_datetime(s, errors="coerce")
                        .dt.tz_localize(None)
                        .dt.normalize())
...
def load_call_data():
    ...
    for fname in settings.call_files:
        ...
        if not date_col:
            log.error(f"{fname}: date column missing")
            continue
...
# ---------------------------------------------------------------------
# 5.  processing/combine.py  (weekday-only dataset)
# ---------------------------------------------------------------------
...
def build_dataset():
    ...
    mail_daily = (mail.groupby("date")["mail_volume"]
                       .sum()
                       .reset_index())
    ...
    if len(df) < settings.min_rows:
        log.error("Too few intersecting weekdays")
        return pd.DataFrame()
...
def save_overview(df, fname="overview.png"):
    ...
    plt.xlabel("Date"); plt.ylabel("0-100")
...
def plot_raw_call_files():
    ...
    plt.title("Raw daily call volume (weekdays)")
    plt.xlabel("Date"); plt.ylabel("Calls")
    plt.legend(); plt.grid(ls="--", alpha=.3)
    ax = plt.gca(); ax.xaxis.set_major_formatter(DateFormatter("%Y-%m-%d"))
    plt.setp(ax.get_xticklabels(), rotation=45, ha="right")
    plt.tight_layout()
    out = settings.output_dir / "raw_call_files.png"
...
def plot_data_gaps():
    calls, _ = load_call_data()
    mail     = load_mail()
    calls = calls[calls["date"].dt.weekday < 5]
    mail  = mail [mail ["date"].dt.weekday < 5]

    if calls.empty and mail.empty:
        return
    ...
    cal["status"] = cal.apply(
        lambda r: "Both"      if r.call and r.mail else
                  "Call only" if r.call          else
                  "Mail only" if r.mail          else "None", axis=1)

    color = {"Both": "green", "Call only": "red",
             "Mail only": "blue", "None": "grey"}

    plt.figure(figsize=(14, 2))
    for s, sub in cal.groupby("status"):
        plt.scatter(sub["date"], [0]*len(sub),
                    c=color[s], s=10, label=s)
    plt.legend(ncol=4, loc="upper center"); plt.yticks([])
    plt.title("Weekday data coverage"); plt.grid(False)
    ax = plt.gca(); ax.xaxis.set_major_formatter(DateFormatter("%Y-%m-%d"))
    plt.setp(ax.get_xticklabels(), rotation=45, ha="right")
    plt.tight_layout()
    out = settings.output_dir / "data_gaps.png"
    plt.savefig(out, dpi=300); plt.close()
    log.info(f"Saved {out}")
PY

# ---------------------------------------------------------------------
# 9.  analytics/eda.py   **patched lag_scan**
# ---------------------------------------------------------------------
...
def validate(df):
    ...
    settings.output_dir.mkdir(exist_ok=True)
    json.dump(rep, open(settings.output_dir / "data_health_report.json",
...
def corr_heat(df):
    ...
    sns.heatmap(num.corr(), annot=True, cmap="vlag", fmt=".2f")
    plt.title("Pearson correlation (weekdays)")
    plt.tight_layout()
    out = settings.output_dir / "corr_heatmap.png"
    plt.savefig(out, dpi=300); plt.close()
    log.info(f"Saved {out}")

# ---------- PATCHED lag_scan (length-safe) ---------------------------
def lag_scan(df, feat="mail_volume", tgt="call_volume"):
    ...
    for lag in lags:
        ...
        valid = s1.notna() & s2.notna() # keep rows where both present
        if valid.sum() < 2:             # too few data points
...
def rolling(df, window=30):
    if len(df) < window + 5:
        return
    r = df.set_index("date")[["call_volume",
                              "mail_volume"]].rolling(window)
    m = r.mean(); s = r.std()
    plt.figure(figsize=(14, 6))
    plt.plot(m.index, m["call_volume"],
             label=f"Call mean {window}", color="tab:red")
    plt.plot(m.index, m["mail_volume"],
             label=f"Mail mean {window}", color="tab:blue")
...
# ---------------------------------------------------------------------
# 10. analytics/corr_extras.py (unchanged)
# ---------------------------------------------------------------------
...
def lag_corr_heatmap(df, max_lag=21):
    ...
    for lag in range(max_lag + 1):
        x = df["mail_volume"]
        y = df["call_volume"].shift(-lag)
        mask = x.notna() & y.notna()
        if mask.sum() < 2:
            vals.append(float("nan")); continue
        r, _ = pearsonr(x[mask], y[mask]); vals.append(r)
    plt.figure(figsize=(8, 4))
    sns.heatmap(np.array(vals).reshape(1, -1), annot=True, fmt=".2f",
                cmap="vlag", cbar=False,
                xticklabels=list(range(max_lag + 1)),
                yticklabels=["r"])
    plt.title("Mail → Call correlation vs lag")
    plt.xlabel("Lag (days)")
    out = settings.output_dir / "lag_corr_heatmap.png"
    plt.tight_layout(); plt.savefig(out, dpi=300); plt.close()
    log.info(f"Saved {out}")
...
def rolling_corr(df, window=30):
    ...
    plt.figure(figsize=(10, 4))
    plt.plot(r.index, r); plt.grid(ls="--", alpha=.4)
    plt.title(f"{window}-day rolling Mail↔Call correlation")
    ax = plt.gca(); ax.xaxis.set_major_formatter(DateFormatter("%Y-%m-%d"))
    plt.setp(ax.get_xticklabels(), rotation=45, ha="right")
    plt.tight_layout()
    out = settings.output_dir / "rolling_corr.png"
    plt.savefig(out, dpi=300); plt.close()
    log.info(f"Saved {out}")
PY

# ---------------------------------------------------------------------
# 11. analytics/mail_intent_corr.py  (unchanged)
# ---------------------------------------------------------------------
...
def top10_mail_intent_corr():
    ...
    if intents.empty or mail.empty:
        return
    mail_piv = mail.pivot_table(index="date",
                                columns="mail_type",
                                values="mail_volume",
                                aggfunc="sum").fillna(0)
    merged = pd.merge(mail_piv.reset_index(), intents,
    ...
    heat = (pd.DataFrame(top, columns=["mail_type", "intent",
                                       "abs_r", "r"])
              .set_index(["mail_type", "intent"]))
    plt.figure(figsize=(8, 6))
    sns.heatmap(heat[["r"]], annot=True, center=0,
                cmap="vlag", fmt=".2f")
    plt.title("Top-10 |r| Mail-Type × Call-Intent (weekdays)")
    plt.tight_layout()
...
# ---------------------------------------------------------------------
# 12. analytics/advanced.py (unchanged)
# ---------------------------------------------------------------------
...
def coverage_report(call_df, mail_df):
    call_dates = set(call_df["date"])
    mail_dates = set(mail_df["date"])
    both       = call_dates & mail_dates
    rep = {"call_dates": len(call_dates),
           "mail_dates": len(mail_dates),
           "intersection": len(both)}
    json.dump(rep, open(settings.output_dir / "call_coverage.json",
                        "w", encoding="utf-8"), indent=2)
...
def ratio_anomaly(df):
    if df.empty:
        return
    df = df.copy()
    df["ratio"] = np.where(df["mail_volume"] > 0,
                           df["call_volume"] / df["mail_volume"],
                           np.nan)
    ...
    plt.scatter(df["mail_volume"], df["call_volume"],
                c=np.where(df["anom"], "red", "grey"), alpha=.6)
    plt.title("Call vs Mail (IsolationForest anomalies)")
    plt.xlabel("Mail"); plt.ylabel("Call")
    plt.grid(ls="--", alpha=.3); plt.tight_layout()
    out = settings.output_dir / "ratio_anomaly.png"
    plt.savefig(out, dpi=300); plt.close()
    log.info(f"Saved {out}")
...
def call_seasonality(call_df):
    ...
    call_df["month"] = call_df["date"].dt.strftime("%b")
    call_df["dow"]   = call_df["date"].dt.day_name()
    piv = (call_df.pivot_table(index="dow",
                               columns="month",
                               values="call_volume",
                               aggfunc="mean")
                   .reindex(index=["Monday","Tuesday",
                                   "Wednesday","Thursday",
                                   "Friday"]))
    plt.figure(figsize=(10, 6))
    sns.heatmap(piv, cmap="Reds",
                annot=True, fmt=".0f")
    plt.title("Average call volume – Month × Weekday")
    plt.tight_layout()
    out = settings.output_dir / "call_seasonality.png"
    plt.savefig(out, dpi=300); plt.close()
    log.info(f"Saved {out}")
PY
...
def main():
    ...
    # Stage 0-4
    save_overview(df); validate(df); corr_heat(df)
    lag_scan(df);      rolling(df)

    # Extras
    plot_raw_call_files(); ratio_anomaly(df); call_seasonality(call_vol)

    # Correlation suite + data gaps
    lag_corr_heatmap(df); rolling_corr(df)
    top10_mail_intent_corr(); plot_data_gaps()
